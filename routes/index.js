            ...
            newuser.save(function(err, savedUser){
                ...
                if(err){
                    ...
                    newProfile.save(function(err, savedProfile){
                        if(err){
                            console.log("LOG: /Register - Could not create the user profile. ");
                            var responseObject = {message : "Error!"};
                            res.status(500).send(responseObject);
                        }
                        else{
...
console.log("Received values are   "+ req.body.eventName
                                +" "+ req.body.eventType
                                +" "+ req.body.seatsAvailable
                                +" "+ req.body.preferences);
            
 console.log("Received values are  "+ req.body.dateDay
                                +" "+ req.body.dateMonth
                                +" "+ req.body.dateYear);

console.log("Received values are   "+ req.body.eventTimeHour
                                +" "+ req.body.eventTimeMinute);
                                
console.log("Received values are   "+ req.body.pickUpTimeHour
                                +" "+ req.body.pickUpTimeMinute);
                                
console.log("Received values are   "+ req.body.eventLocationLat
                                +" "+ req.body.eventLocationLng);

console.log("Received values are   "+ req.body.eventLocation
    ...
    newEvent.save(function(err, savedEvent){
        ...
        if(err){
            console.log("LOG: /CreateEvent - Could not create the event");
            console.log(err);
            var responseObject = {message : "Error!"};
...
router.put('/request', checkAuth, function(req, res){
    ...
    console.log("Received values are "+ req.body.eventid
                                +" "+ req.body.status
                    ...
                    Event.findOne({_id: eventid }, function (err, myDocument) {
                        ...
                        else
                        {
                            ...
                            Event.update({_id: eventid}, condition_1,function (err, updated) {
                                            ...
                                            if(err)
                                                {
                                                  console.log("Could not insert remaining");
                                                }
                                });   
                    }
               });
        ...
        if(err){
            ...
            if(request!=null){
                ...
                Requests.update({_id: requestid}, updatevalues, function(err, done){
                    ...
                    if(err){
                        console.log("Could not update seen");
                        res.status(500).jsonp({message : "Error!"});
                    }else{
                        res.status(200).jsonp({message: "Successfully updated"});
                    }
